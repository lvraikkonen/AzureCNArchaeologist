...
Purview 另有用户确认的代码级追加规则，按 Batch/Review 已记录的 Product Key 精确启用，见 [Complex 源内容边界 §4.3](m3-strategy-boundaries.md#43-purview-的明确交付规则)。L3b 从原始源节点独立形成三个主类别的正文，每组末尾包括应用程序说明和两个应用面板内部内容，并单独核对数据映射共享说明。删除任一追加表格、颠倒顺序、重新带入已取消的页签包装或漏掉共享说明都必须失败；生产追加函数失效时，L3b 仍须发现遗漏。该规则不进入全局 HTML 规范化，也不适用于其他 Product Key。
