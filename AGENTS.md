...
```
Source Snapshot → PipelineCoordinator（发现、标准化、预检）
→ BatchProcessEngine（资源级并行）→ ExtractionCoordinator
→ StrategyManager / StrategyFactory → 具体策略类（提取）
→ persisted-payload 验证 → Review Queue → Batch Report
```
...
`pipeline-run` 默认要求 clean worktree，语言默认 `both`；只有明确接受不可复现批次时才使用 `--allow-dirty`。pipeline 不执行上传或发布。
...
├── runs/               # 每个 Batch Run 的 manifest、产物、验证、审核队列、日志和报告
└── output/             # 单产品 extract 命令的兼容输出目录
```

Pipeline 的 canonical 路径固定为：

- Pricing payload：`runs/{batch_id}/outputs/{language}/pricing/{resource}.json`；
- Support Article payload：`runs/{batch_id}/outputs/{language}/SupportArticles/{articleType}/{resource}.json`；
- Diagnostic sidecar：在 `runs/{batch_id}/diagnostics/` 下使用与 payload 相同的分类目录，并以 `{resource}.sidecar.json` 命名；
- Validation 投影：在 `runs/{batch_id}/validation/` 下使用与 payload 相同的分类目录，并以 `{resource}.validation.json` 命名；
- Review Queue 与日志：`runs/{batch_id}/review/review-queue.json`、`runs/{batch_id}/logs/pipeline.jsonl`。

Pricing 无论属于多少 catalog category 都只写入 `pricing/`，category 仅作为元数据，不参与产物路径。
