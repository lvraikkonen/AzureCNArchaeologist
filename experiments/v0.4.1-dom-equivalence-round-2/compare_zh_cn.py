#!/usr/bin/env python3
"""Compare round-two v0.4.1 zh-cn CMS fragments with an independent DOM oracle.
...
The method and artifact schema are frozen from commit 048cf07's first-round
``v0.4.1-dom-equivalence`` experiment.  This copy changes only the experiment
identity, product cohort, controlled-mutation subjects, and source locations
needed by products that do not have normalized inputs.
"""
...
EXPERIMENT_NAME = "v0.4.1-dom-equivalence-zh-cn-round-2"
...
class ProductSpec:
    ...
    def source_relative_path(self) -> str:
        if self.product_key in {"cdn", "data-transfer"}:
            return (
                f"data/current_prod_html/{LANGUAGE}/pricing/details/"
                f"{self.product_key}/index.html"
            )
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# v0.4.1 第二轮中文 DOM 与 CMS payload 对比实验",
        "",
        (
...
<title>v0.4.1 第二轮 DOM 与 CMS payload 人工复核</title>
<style>
...
<body>
<h1>v0.4.1 第二轮中文 DOM 与 CMS payload 人工复核</h1>
...
def run(
    ...
    *,
    ...
    report = {
        ...
        "method_reference": {
            "commit": "048cf07",
            "experiment": "experiments/v0.4.1-dom-equivalence",
        },
        "inputs": {
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Compare round-two v0.4.1 zh-cn frozen DOM fragments with CMS payloads"
        )
    )
    parser.add_argument(
        "--extractor-output",
