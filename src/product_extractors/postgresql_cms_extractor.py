...
# -*- coding: utf-8 -*-
"""
Azure Database for PostgreSQL页面CMS导入HTML提取器 (重构版)
基于模块化架构，继承BaseCMSExtractor提供PostgreSQL特定功能
"""
...
class PostgreSQLCMSExtractor(BaseCMSExtractor):
    """Azure Database for PostgreSQL页面CMS HTML提取器 - 重构版"""
    
    def __init__(self, config_file: str = "soft-category.json", 
                 output_dir: str = "postgresql_output"):
        """
        初始化PostgreSQL CMS提取器
        
        Args:
            config_file: 配置文件路径
            output_dir: 输出目录
        """
        super().__init__(config_file, output_dir, "Azure Database for PostgreSQL")
    
    @property
    def important_section_titles(self) -> Set[str]:
        """获取PostgreSQL重要的section标题集合"""
        return {
    ...
    def get_product_specific_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "banner_class": "postgresql-product-banner",
            "content_class": "postgresql-pricing-content"
        }
    ...
    def get_css_styles(self, region_name: str) -> str:
        ...
        .postgresql-pricing-table th,
        .postgresql-pricing-table td {
        ...
        .postgresql-pricing-table tr:nth-child(even) {
    ...
    def save_cms_html(self, result: Dict[str, Any], region: str, custom_filename: str = "") -> str:
        """保存PostgreSQL CMS HTML文件（重写以添加PostgreSQL特定信息）"""
        ...
        if "postgresql_keywords_found" in verification:
            keywords = verification["postgresql_keywords_found"]
            print(f"🔍 检测到PostgreSQL关键词: {keywords}")
        
        if "postgresql_table_count" in verification:
            table_count = verification["postgresql_table_count"]
            print(f"📊 PostgreSQL定价表: {table_count} 个")
        
        # 显示服务器类型信息
        if "server_types_found" in verification:
            server_types = verification["server_types_found"]
            print(f"🗄️ 检测到服务器类型: {server_types}")
        
        # 显示定价层信息
        if "pricing_tiers_found" in verification:
            pricing_tiers = verification["pricing_tiers_found"]
            print(f"⚡ 检测到定价层: {pricing_tiers}")
...
def main():
    """主程序入口"""
    parser = argparse.ArgumentParser(description="Azure Database for PostgreSQL页面CMS HTML提取器 (重构版)")
    parser.add_argument("html_file", help="PostgreSQL HTML源文件路径")
    parser.add_argument("-r", "--region", default="east-china3", help="目标区域")
    ...
    parser.add_argument("-o", "--output", default="postgresql_output", help="输出目录")
    ...
    try:
        # 创建PostgreSQL CMS提取器
        extractor = PostgreSQLCMSExtractor(args.config, args.output)
        
        # 显示版本信息
        print("🚀 Azure Database for PostgreSQL页面CMS HTML提取器 v2.0 (重构版)")
