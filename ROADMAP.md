...
> 文档状态：当前项目路线图  
> 最新稳定版本：v0.5.5
> 当前开发阶段：accepted functional baseline 为 v0.5.5；v0.5.6 已在实施前 supersede；当前进入 Repository Rebaseline preparation
> 基线日期：2026-08-13
...
| v0.5.6 | `superseded before implementation` | Plan freeze 后的新人工 Source 检查推翻 repair authority；无 implementation、Batch、Evidence、版本或 tag |
| Repository Rebaseline（pre-v0.6） | **next active phase：preparation** | 当前不执行审查或重构 |
| v0.6 | blocked / unplanned / unauthorized | entry condition 为 Repository Rebaseline accepted；新基线形成前 scope intentionally undefined |
...
已完成的顺序是：v0.5.0 由既有实验关闭可行性问题；v0.5.1 建立入口基线并冻结最小正式契约；v0.5.2 为 `zh-cn/api-management` 建立单项首个正式 L3b 证据闭环和只读并排报告；v0.5.3 完成双语四类 Core 8、两个 carry-over qualification、现有 Workbench Evidence panel，并接受 `parallel_only` / `runtime_effective=false` Machine Gate decision；v0.5.4 通用修复 SupportArticle direct-text preservation，并以新 Batch、三项 passed Evidence、全 family 回归和不可变历史负证据完成验收；v0.5.5 完成 Simple boundary 并成为当前 accepted functional baseline。v0.5.6 在 Plan freeze 后、implementation 前因新 Source 事实被 supersede；当前进入 Repository Rebaseline preparation，不直接进入 v0.6。
...
> 状态：**`superseded before implementation`（2026-08-13）**。原 Plan freeze commit 为 `7e8ab83bbd58cd6eed1b943bc83ff555c8b229e7`；无 production implementation、formal Batch、canonical Evidence、版本升级或 `v0.5.6` tag。权威裁决见 `reports/v0.5.6/supersession-decision.md`。

Plan freeze 后进一步人工检查双语 `postgresql`、`hpc-cache` 与 `app-configuration`，确认当前失败主要来自历史遗留结构、异常嵌套、错误或无效控件及实际页面与 Source 不一致；上游 Source HTML 团队已确认将修正这些输入。原 repair 6、F1–F4、20-scope Evidence/Workbench design 与 329/54 projection 因此前提失效，不再具有活动实施授权，也不自动继承到任何未来 preflight。

六个 language items 的当前 disposition 精确为：

| item | disposition | owner | current behavior | re-entry |
|---|---|---|---|---|
| `en-us/postgresql` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |
| `zh-cn/postgresql` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |
| `en-us/hpc-cache` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |
| `zh-cn/hpc-cache` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |
| `en-us/app-configuration` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |
| `zh-cn/app-configuration` | `upstream-source-correction-pending` | upstream Source HTML team | 保持失败；无新 payload | corrected Source 后人工核对 + new preflight |

不为这六项新增产品特例或通用 Source 容错。F4 canonical/legacy reader duplication 只进入 Repository Rebaseline finding candidates，不预设删除、合并或修改。双语 `firewall-manager` 继续保持 active repair scope 外的 upstream Source blockers，不因本次裁决重新纳入 extractor repair。

accepted functional baseline 继续是 annotated tag `v0.5.5` → `fdf4461602875fcd97c8556b01118c1114e7c7b3`，accepted Batch 为 `20260813T113000Z-b819c3f2`。v0.5.6 原 31/6/4/21 accounting 只保留在 superseded 历史 Plan 中，不再是活动 implementation denominator；其他 residuals 也不自动归属 v0.6。

### Repository Rebaseline / 代码库重基线（pre-v0.6）

> 状态：**next active phase — preparation**。审查宪章，当前只准备文档，尚未开始审核，也不授权重构或代码修改。

重基线固定使用 v0.5.5 accepted code、inputs、Batch 与 Evidence。期间不得引入 corrected PostgreSQL、HPC Cache、App Configuration 或 Firewall Manager Source，避免代码变化与 Source 输入变化同时发生。
...
### v0.6：blocked / unplanned / unauthorized
...
→ v0.5.5 完成 Simple 安全修复并成为 accepted functional baseline
→ v0.5.6 Plan 在 implementation 前因新 Source 事实被 supersede
→ Repository Rebaseline preparation、只读审查、finding disposition、accepted refactor plan 与新基线
...
- v0.5.4–v0.5.5 的具体问题组由 v0.5.3 当前完整双语 Batch、47/47 scope 人工审核和 accepted residual problem map 提供历史依据；v0.5.6 Plan 已在 implementation 前 supersede，其 residuals 不预先绑定 v0.6，须在 Repository Rebaseline 的新 accepted baseline 上重新排序。旧 C1–C9 数量只作为历史比较。
