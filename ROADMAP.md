...
Azure Blob Release 发布已于 2026-08-26 按真实 CMS 交付约定实现，不再属于待评估事项。当前实现以 `delivery-manifest.json` 为完成标志；CMS 端的消费逻辑仍由下游团队负责。
...
| 2026-08-26 | 实现完整与增量 Release 的 Azure Blob 发布：按上传日期和 Release ID 分目录，全部 Payload 成功后最后发布 `delivery-manifest.json`，连接串只从 `.env` 或进程环境读取 |
