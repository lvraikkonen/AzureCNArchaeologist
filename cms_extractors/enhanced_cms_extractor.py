...
class EnhancedCMSExtractor:
    ...
    def extract_cms_content(self, html_file_path: str, url: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            content_fields = [
                "DescriptionContent", "BannerContent", "QaContent", "NoRegionContent",
    ...
    def _extract_region_contents(self, soup: BeautifulSoup, html_file_path: str = "") -> Dict[str, str]:
        ...
        # 初始化所有区域内容为空
        for region_key in self.region_mapping.values():
            region_contents[region_key] = ""
        
        # 查找每个区域的内容
        for region_id, content_key in self.region_mapping.items():
            content = self._extract_single_region_content(soup, region_id, html_file_path)
            if content:
                region_contents[content_key] = content
                print(f"  ✓ 提取 {self.region_names[region_id]} 内容: {len(content)} 字符")
        ...
        for selector in main_content_selectors:
            ...
            if elements:
                for element in elements:
                    # 复制元素到内容容器
                    content_soup.append(BeautifulSoup(str(element), 'html.parser'))
                break
