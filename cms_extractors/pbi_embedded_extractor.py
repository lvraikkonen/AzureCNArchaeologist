...
# -*- coding: utf-8 -*-
"""
Power BI Embedded页面CMS导入HTML提取器 (重构版)
基于模块化架构，继承BaseCMSExtractor提供Power BI Embedded特定功能
"""
...
class PowerBIEmbeddedCMSExtractor(BaseCMSExtractor):
    """Power BI Embedded页面CMS HTML提取器 - 重构版"""
    
    def __init__(self, config_file: str = "soft-category.json", 
                 output_dir: str = "power_bi_embedded_output"):
        """
        初始化Power BI Embedded CMS提取器
        
        Args:
            config_file: 配置文件路径
            output_dir: 输出目录
        """
        super().__init__(config_file, output_dir, "Power BI Embedded")
    
    @property
    def important_section_titles(self) -> Set[str]:
        """获取Power BI Embedded重要的section标题集合"""
        return {
    ...
    def get_product_specific_config(self) -> Dict[str, Any]:
        """获取Power BI Embedded产品特定配置"""
        return {
            "table_class": "power-bi-embedded-pricing-table",
            "banner_class": "power-bi-embedded-product-banner",
            "content_class": "power-bi-embedded-pricing-content"
        }
    
    def extract_product_banner(self, soup: BeautifulSoup, content_soup: BeautifulSoup) -> List:
        """提取Power BI Embedded产品横幅"""
    ...
    def get_css_styles(self, region_name: str) -> str:
        ...
        .power-bi-embedded-pricing-table {
            width: 100%;
        ...
        .power-bi-embedded-pricing-table th,
        .power-bi-embedded-pricing-table td {
        ...
        .power-bi-embedded-pricing-table tr:nth-child(even) {
            background-color: #f9f9f9;
        }
        
        /* Power BI Embedded特有样式 */
        .node-type-section {
            margin-top: 2rem;
            padding: 1rem;
    ...
    def save_cms_html(self, result: Dict[str, Any], region: str, custom_filename: str = "") -> str:
        """保存Power BI Embedded CMS HTML文件（重写以添加特定信息）"""
        ...
        # 显示Power BI Embedded特定的验证信息
        verification = result.get("verification", {})
        
        if "power_bi_keywords_found" in verification:
            keywords = verification["power_bi_keywords_found"]
            print(f"📊 检测到Power BI关键词: {keywords}")
        
        if "node_types_found" in verification:
            nodes = verification["node_types_found"]
            print(f"🖥️ 检测到节点类型: {nodes}")
        
        if "pricing_components_found" in verification:
            components = verification["pricing_components_found"]
            print(f"💰 检测到定价组件: {components}")
...
def main():
    """主程序入口"""
    parser = argparse.ArgumentParser(description="Power BI Embedded页面CMS HTML提取器 (重构版)")
    parser.add_argument("html_file", help="Power BI Embedded HTML源文件路径")
    parser.add_argument("-r", "--region", default="east-china2", help="目标区域")
    ...
    parser.add_argument("-o", "--output", default="power_bi_embedded_output", help="输出目录")
    ...
    try:
        # 创建Power BI Embedded CMS提取器
        extractor = PowerBIEmbeddedCMSExtractor(args.config, args.output)
        
        # 显示版本信息
        print("🚀 Power BI Embedded页面CMS HTML提取器 v2.0 (重构版)")
        ...
        print("\n🎉 Power BI Embedded CMS HTML提取任务完成！")
