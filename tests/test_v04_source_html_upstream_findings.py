...
EXPECTED_BLOCKING_STRUCTURE_COUNTS = {
    "SOURCE_HTML_COMMON_SECTION_BOUNDARY_NOT_EXACT": 2,
...
def test_confirmed_and_review_only_products_remain_separate() -> None:
    ...
    assert report["survey"]["canonical_sources_surveyed"] == 372
    assert report["survey"]["simple_static_sources_surveyed"] == 66
    assert report["summary"] == {
        "confirmed_product_keys": ["virtual-wan"],
        "confirmed_language_findings": 2,
        ...
        "blocking_structure_language_items": 8,
        "blocking_structure_findings": 9,
    ...
    assert all(
        ...
        and item["payload_generation_allowed"] is False
        for item in report["confirmed_blocking_findings"]
    )
    assert all(
    ...
    assert len(blocking_structure) == 9
    assert {
        (item["product_key"], item["language"])
