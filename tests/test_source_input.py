...
def test_real_scope_freezes_all_declared_files_with_identical_bytes(
    tmp_path: Path,
) -> None:
    catalog = ProductCatalog.load(PROJECT_ROOT)
    ...
    assert report.succeeded
    assert report.selected_product_count == 184
    assert report.selected_item_count == 368
    assert report.passed_item_count == 368
    for item in items:
