...
8. 对已确认不再维护的移动控件，只在桌面默认项唯一且显示一致时采用桌面默认项；移动端重复或冲突的默认标记不参与判断，也不单独产生警告，桌面版自身不明确时仍停止抽取。
...
补充进展：2026-08-08 完成了中文 `service-bus`、`api-management`、`app-service`、`sla-virtual-machines` 四产品预实验，19 个目标片段原始字符串全部一致，并成功发现一次受控的错状态内容。报告见 `reports/post-v0.4/zh-cn-dom-payload-experiment.md`。这项结果说明方向可行，但尚未绑定 accepted v0.4.1 Batch，也不替代上面的完整样例矩阵。
...
3. 除 v0.4.1 已处理的“桌面默认项明确、移动版重复标记”情况外，对 C4 只做归因和拆分：形成子问题地图、优先级和代表样例，实际修复进入 v0.6。
