...
| Task | 状态 | 内容 | 主要产物 |
|---|---|---|---|
| M7-01 | 待开始 | 运行完整自动化测试 | 测试报告 |
| M7-02 | 待开始 | 运行 22 产品、44 处理项真实双语回归 | 全量回归报告 |
| M7-03 | 待开始 | 运行单产品、management Category、全量和增量演练 | 四类运行记录 |
| M7-04 | 待开始 | 验证四种 Strategy 的 L3a、L3b 和人工审核样例 | 支持证据 |
| M7-05 | 待开始 | 关闭所有会造成静默丢失、错状态、跨产品污染或错误发布的问题 | 缺陷清单 |
| M7-06 | 待开始 | 核对 README、词汇表、规格、路线图和 CLI 帮助与实现一致 | 文档检查 |
| M7-07 | 待开始 | 确认凭据、连接信息和用户隐私数据不会进入仓库或日志 | 安全检查 |
| M7-08 | 待开始 | 记录每个首批产品的新支持、阻断或待确认结论 | 支持矩阵 |
| M7-09 | 待开始 | 构建并人工复核 v1.0 候选 Release | Release Candidate |
| M7-10 | 待开始 | 根据全部验收结果决定是否标记 v1.0 | 发布决定 |
